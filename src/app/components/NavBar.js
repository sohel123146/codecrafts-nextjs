...
const NavBar = () => {
  ...
  return (
    <div className='flex flex-col justify-center items-center px-1 py-1 md:mt-0 mt-0 m-4 md:flex-row md:justify-start md:items-center shadow-lg bg-gray-200 sticky top-0 z-10'>
      ...
      <div onClick={togglecart} className="cart absolute md:mr-8 top-5 right-4 md:top-5 cursor-pointer text-3xl md:text-2xl">
        <CiShoppingCart />
      </div>

      <div ref={ref} className='w-72 h-[100vh] sidecart absolute top-0 -right-4 bg-gray-200 border border-1 px-8 py-12 rounded-2xl transform transition-transform translate-x-full'>
        ...
        <ol className='list-decimal flex flex-col'>
          ...
          {Object.keys(cart).map((k)=>{return <li key={k}>     {/* Object.keys() converts keys into array */}
            <div className='item flex justify-between items-center my-4'>
              <div className='text-sm'>{cart[k].name}</div>
              <div className=' border border-1 flex flex-row justify-between items-center'><FaMinusCircle onClick={()=>removeFromCart(k,1,cart[k].price,cart[k].name,cart[k].size,cart[k].variant)} className='mx-2 cursor-pointer' />{cart[k].qty}<FaPlusCircle onClick={()=>addToCart(k,1,cart[k].price,cart[k].name,cart[k].size,cart[k].variant)} className='mx-2 cursor-pointer' /></div>
            </div>
          </li>})}
        </ol>
        ...
        <div className='flex'>
          ...
          <button onClick={clearCart} className="flex ml-2 items-center mt-2 text-white bg-indigo-500 border-0 py-3 px-3 focus:outline-none hover:bg-indigo-600 rounded text-sm">
          Clear Cart
          </button>
        </div>
      </div>
